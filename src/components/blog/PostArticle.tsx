      ...
      <header className={`${preview ? "pt-12" : "pt-32"} pb-10 px-6 md:px-12 lg:px-24`}>
        <div className="text-steel text-[10px] tracking-[0.3em] flex flex-wrap items-center gap-3 mb-8">
          {preview ? (
            ...
            <PublicLink href="/" intent="section" className="hover:text-bone">{siteName}</PublicLink>
          )}
          <span>/</span>
          {preview ? (
        ...
        <footer className="px-6 md:px-12 lg:px-24 py-12 border-t border-iron">
          {related.length > 0 ? (
            <div className="mb-12">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {related.map((item) => (
                  <PublicLink key={item.slug} href={`/blog/${item.slug}`} intent="sibling-next" className="border border-iron p-4 hover:border-ember transition-colors">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-6 text-[10px] tracking-[0.2em]">
            ...
            <div className="flex gap-6">
              {previous ? <PublicLink href={`/blog/${previous.slug}`} intent="sibling-prev" className="text-ash hover:text-ember">PREV: {previous.title}</PublicLink> : null}
              {next ? <PublicLink href={`/blog/${next.slug}`} intent="sibling-next" className="text-ash hover:text-ember">NEXT: {next.title}</PublicLink> : null}
            </div>
          </div>
        </footer>
      ) : null}
